...
const ConverterUI = (() => {
  ...
  /**
   * Absolute temperature scales require an affine (offset + scale) conversion.
   * Returns the converted value, or null if not a recognized temperature pair.
   * Recognised symbols: K, °C, °F, °R
   */
  function tryTemperatureConvert(value, srcSym, tgtSym) {
    // Convert everything to Kelvin first, then to target
    const toKelvin = { 'K': v => v, '°C': v => v + 273.15, '°F': v => (v + 459.67) * 5 / 9, '°R': v => v * 5 / 9 };
    const fromKelvin = { 'K': k => k, '°C': k => k - 273.15, '°F': k => k * 9 / 5 - 459.67, '°R': k => k * 9 / 5 };
    if (toKelvin[srcSym] && fromKelvin[tgtSym]) {
      return fromKelvin[tgtSym](toKelvin[srcSym](value));
    }
    return null;
  }
  ...
  function doConvert() {
    ...
    try {
      // Special-case: affine temperature conversion (°C, °F, K, °R)
      const tempResult = tryTemperatureConvert(value, srcExpr.trim(), tgtExpr.trim());
      if (tempResult !== null) {
        const tgtInfo = Engine.computeFactorAndDim(tgtExpr);
        const fakeResult = { value: tempResult, tgtDim: tgtInfo.dim, simplified: null };
        renderResult(outputEl, value, srcExpr, tgtExpr, fakeResult);
